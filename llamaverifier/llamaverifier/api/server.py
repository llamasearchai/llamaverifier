...
async def generate_proof(
    ...
    if proving_key_id not in proving_keys:
        raise HTTPException(status_code=404, detail="Proving key not found. Run setup first.")
    
    try:
        # Generate proof
        proof_path, public_inputs_path = proof_system.generate_proof(
            circuits[circuit_id],
            proving_keys[proving_key_id],
            witness_values,
            scheme
        )
...
async def verify_proof(request: VerificationRequest, scheme: str = "g16"):
    ...
    try:
        ...
        if verification_key_id not in verification_keys:
            raise HTTPException(status_code=404, detail="Verification key not found. Run setup first.")
