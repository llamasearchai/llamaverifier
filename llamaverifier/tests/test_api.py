...
from llamaverifier.api.server import VerificationRequest, app
...
class TestAPIServer(TestCase):
    ...
    def test_compile_endpoint(self, mock_compiler_class):
        ...
        with open(self.model_path, "rb") as model_file:
            ...
            response = self.client.post(
                "/compile",
                files={"model_file": ("model.txt", model_file, "text/plain")},
                data={"model_type": "generic", "optimization_level": "1"},
            )
    ...
    def test_setup_endpoint(self, mock_proof_system_class):
        ...
        # Test the endpoint
        response = self.client.post(f"/setup/{circuit_id}", json={"scheme": "groth16"})
    ...
    def test_verify_endpoint(self, mock_proof_system_class):
        ...
        from llamaverifier.api.server import proofs, public_inputs, verification_keys
    ...
    def test_export_verifier_endpoint(self, mock_proof_system_class):
        ...
        # Test the endpoint
        response = self.client.post(
            f"/export-verifier/{verification_key_id}", json={"scheme": "groth16"}
        )
    ...
    def test_verify_model_endpoint(self, mock_proof_system_class, mock_compiler_class):
        ...
        with open(self.model_path, "rb") as model_file:
            response = self.client.post(
                "/verify-model",
                files={"model_file": ("model.txt", model_file, "text/plain")},
                data={"request": json.dumps(verification_request.dict())},
            )
